...
var id = params.get("id") || "2535409817767873";
var url = "https://api.ngmc.co/v1/players/";
function spl(n) {
	n = "" + n + "";
	return n.split("", 9);
}
...
function search() {
	const prompt = window.prompt("Enter player name or ID.");
	if (prompt)
		fetch(
			`https://api.ngmc.co/v1/players/${prompt}`
		)
			.then((res) => res.json())
			.then((data) => {
				window.location.href = "?id=" + data.xuid;
			});
}
